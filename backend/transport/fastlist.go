...
func queryFastList(deps *Dependencies, r *http.Request, body []byte, debug bool) ([]interface{}, *fastListDebugInfo) {
	...
	if req.Where != nil {
		for _, rs := range req.Where.ResourceSets {
			extras = append(extras, fastlist.FlattenFastListConditions(rs.Condition, db)...)
		}
	}
	limit := req.Limit
	if limit <= 0 {
	...
	if err != nil {
		...
		if chData := fastlist.ChQueryFastList(deps.CH, db, resolvedTbl, selStr, extras, req.TimeStart, req.TimeEnd, limit, req.Offset); chData != nil {
			if !debug {
				return chData, nil
			}
			data := chData
...
func buildFastListDebug(deps *Dependencies, r *http.Request, body []byte, di *fastListDebugInfo) []interface{} {
	...
	for _, t := range tagNames {
		returnTags = append(returnTags, fastlist.QuerierColumn{
			Name:        t,
			DisplayName: t,
			IsResource:  false,
		})
	...
	returnMetrics = append(returnMetrics, fastlist.QuerierColumn{
		Name:        "Count(行数)",
